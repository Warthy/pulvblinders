...
export default class EventsAssociationsScreen extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            error: 'n',
    ...
    getEvents() {
        ...
        axios.get(API.endpoints.events.associations)
            .then(res => {
    ...
    _events = () => this.state.events.map((dayEvents, index, events) => {
        ...
        return (
            <View key={"event-"+index} style={viewStyle}>
                <View key={"view-date"} style={styles.dateHeader}>
                    <Text key={"text-1"} style={styles.dayName}>{dayNames[dayEvents.date.format('d')]}</Text>
                    <Text key={"text-2"} style={styles.dayNumber}>{dayEvents.date.format('DD')}</Text>
                    <Text key={"text-3"} style={styles.dayMonth}>{monthNames[dayEvents.date.format('M')-1]}</Text>
                </View>
                <View key={"view-event"} style={styles.event}>
